...
/**
 * Business rules currently only allow for one service per account to track usage. Therefore there should only be one
 * active service per account. If this changes then much more complicated logic will have to be implemented across the
 * application.
 * 
 */
public class ProvisionSystem extends OldBillService {
	private static final String USERNAME = "TSCPMVNE.ProvisioningSystem";

	/**
	 * Adds a component to the service and package and activates it along with all other components in the package.
	 * 
	 * @param accountNo
	 * @param externalId
	 * @param pkg
	...
	public void addComponent(
			int accountNo, String externalId, Package pkg, Component component) throws ProvisionException {
	...
	/**
	 * Adds the given package to the account and returns the package with it's instanceId populated.
	 * 
	 * @param accountNumber
	 * @param pkg
	 * @return
	...
	public Package addPackage(
			int accountNumber, Package pkg) throws ProvisionException {
	...
	/**
	 * Adds a new service instance to the given account. If the external ID already exists on the account the call to
	 * service.addService will fail from a SQL constraint.
	 * 
	 * Error adding ServiceInstance ########## to account ######. Error::ORA-20001: 316012, TRIG: INSERT/UPDATE Failed:
	 * Overlapping active/inactive dates for external_id_type marked unique in EXTERNAL_ID_TYPE_REF ORA-06512: at
	 * "ARBOR.EXTERNAL_ID_EQUIP_MAP_ATRIG"
	 * 
	 * @param accountNo
	 * @param externalId
	 * @throws ProvisionException
	...
	public void addServiceInstance(
			int accountNo, String externalId) throws ProvisionException {
		ProvisionUtil.checkAccountNumber(accountNo);
		ProvisionUtil.checkExternalId(externalId);
	...
	/**
	 * Adds a single component to the package and service without activating all other components in the package.
	 * 
	 * @param accountNumber
	 * @param externalId
	 * @param pkg
	...
	public void addSingleComponent(
			int accountNumber, String externalId, Package pkg, Component component, DateTime activeDate) throws ProvisionException {
		ProvisionUtil.checkAccountNumber(accountNumber);
		ProvisionUtil.checkExternalId(externalId);
		ProvisionUtil.checkPackage(pkg);
	...
	public List<Component> getActiveComponents(
			int accountNumber, String externalId) throws ProvisionException {
		try {
	...
	public void removeComponent(
			int accountNo, String externalId, Package pkg, Component component, DateTime dateTime) throws ProvisionException {
		ProvisionUtil.checkAccountNumber(accountNo);
		ProvisionUtil.checkExternalId(externalId);
		ProvisionUtil.checkPackage(pkg);
	...
	protected void removePackage(
			int accountNo, Package pkg) throws ProvisionException {
		ProvisionUtil.checkAccountNumber(accountNo);
		ProvisionUtil.checkPackage(pkg);
	...
	public void removeServiceInstance(
		...
		ProvisionUtil.checkAccountNumber(accountNo);
		ProvisionUtil.checkExternalId(externalId);
		MessageHolder message = port.disconnectServicePackages(USERNAME, Integer.toString(accountNo), externalId, PROVISION.SERVICE.EXTERNAL_ID_TYPE, DateUtils.getXMLCalendar(), DISC_REASON);
		try {
			ProvisionUtil.checkResponse(message);
		} catch (BillingServerException e) {
