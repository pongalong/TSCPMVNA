...
public final class ProvisionUtil extends BillingUtil {
	...
	public static final boolean isCurrentMonth(
			Component component) {
	...
	/**
	 * Returns a Package as required by the Billing Server or the default package if none is specified.
	 * 
	 * @param pkg
	 * @return
	 */
	public static final com.telscape.billingserviceinterface.Package buildBillingPackage(
			int accountNo, Package pkg) {
		com.telscape.billingserviceinterface.Package billingPackage;
		if (pkg == null) {
			billingPackage = new DefaultBillingPackage();
			billingPackage.setAccountNo(Integer.toString(accountNo));
		} else {
	...
	public static final Component buildComponent(
	...
	public static final ArrayOfPkgComponent buildComponentList(
			String externalId, Package pkg, Component component) {
	...
	public static final ArrayOfPackage buildPackageList(
			int accountNo, Package pkg) {
	...
	public static final Package buildPackage(
			com.telscape.billingserviceinterface.Package billingPackage) {
	...
	public static final PkgComponent buildPkgComponent(
			String externalId, Package pkg, Component component) {
	...
	public static final void checkComponent(
			Component component) throws ProvisionException {
		if (component.getId() == 0) {
			throw new ProvisionException("Component ID is not set");
		}
	}

	public static final void checkExternalId(
			String externalId) throws ProvisionException {
		if (externalId == null || externalId.trim().isEmpty()) {
			throw new ProvisionException("External ID is not set");
		}
	}

	public static final void checkPackage(
			Package pkg) throws ProvisionException {
		if (pkg == null || pkg.getInstanceId() == 0) {
			throw new ProvisionException("Package is not set");
		}
	}

	public static final Date getServiceDate(
			String serviceDate) {
	...
	@Deprecated
	public static final void syncPackages(
			Package pkg, com.telscape.billingserviceinterface.Package billingPackage) {
		if (pkg.getId() > 0) {
			billingPackage.setPackageId(pkg.getId());
		} else {
			pkg.setId(billingPackage.getPackageId());
	...
	public static final String toServiceDate(
			Date date) {
