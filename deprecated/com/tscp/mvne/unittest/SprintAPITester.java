...
public class SprintAPITester {
	...
	public static void main(
			String[] args) throws NetworkQueryException {
	...
	private void retrieveValidNPAList(
			String csa) {
	...
	private void getSubscriberInfo(
		...
		if (subscription != null) {
			if (subscription.getAccessEqpAsgmList() != null && subscription.getAccessEqpAsgmList().getValue() != null && subscription.getAccessEqpAsgmList().getValue().size() > 0) {
	...
	private void expireSubscription(
			String MDN) throws DisconnectException, NetworkQueryException {
		...
		OldNetworkService networkImpl = new OldNetworkService();

		OldNetworkInfo networkInfo = networkImpl.getNetworkInfo(null, MDN);
		...
		networkImpl.disconnect(networkInfo);

	}

	public void reserveMdn() throws ReserveException {
		OldNetworkService networkImpl = new OldNetworkService();
		OldNetworkInfo networkInfo = networkImpl.reserveMdn(null, null, null);
	...
	public void activateMdn() throws NetworkQueryException {
		OldNetworkService networkImpl = new OldNetworkService();
		...
		networkImpl.connect(networkInfo);
