...
public class Service {
	...
	public void connect(
		...
		logger.debug(ClassUtils.toString(newServiceInstance));

		newServicePackage = ProvisionServicePackageService.addPackage(account.getAccountNo(), new ServicePackage());
		logger.debug(ClassUtils.toString(newServicePackage));
		...
		newServiceComponent = ProvisionServiceComponentService.addComponent(newServiceComponent);
		logger.debug(ClassUtils.toString(newServiceComponent));
	...
	public void restore() throws ServiceIntegrityException, ServiceRestoreException, ProvisionException {
		...
		if (getActiveServiceComponent().getActiveDate().getDayOfMonth() == new DateTime().getDayOfMonth() + 1)
			ProvisionServiceComponentService.removeComponentNextDay(getActiveServiceComponent());
		else
			ProvisionServiceComponentService.removeComponent(getActiveServiceComponent());

		ProvisionServiceComponentService.addSingleComponent(restoreComponent);
	}
	...
	public void suspend() throws ServiceIntegrityException, ServiceSuspendException, ProvisionException {
		...
		ProvisionServiceComponentService.removeComponentNextDay(getActiveServiceComponent());
		ProvisionServiceComponentService.addSingleComponentNextDay(new SuspendComponent(getActiveServiceComponent()));
	}
