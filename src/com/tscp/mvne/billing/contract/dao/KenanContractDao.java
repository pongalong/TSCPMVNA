...
public class KenanContractDao {
	...
	public static int insertContract(
			KenanContract contract) throws DaoException {
		...
		if (response != null && response.isSuccess()) {
			...
			throw new DaoException("Error inserting contract " + contract.getContractType() + " on account " + contract.getAccount().getAccountNo() + ". Fail Reason is : " + response.getMsg());
		}
	}
	...
	public static int applyCouponPayment(
			Account account, String amount, Date date) throws DaoException {
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		Transaction transaction = session.beginTransaction();
		...
		if (response != null && response.isSuccess()) {
			...
			throw new DaoException("Error applying coupon payment on account " + account.getAccountNo() + " for " + amount + ". Fail Reason is : " + response.getMsg());
		}
	}
