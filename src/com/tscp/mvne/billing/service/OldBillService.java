...
import com.tscp.mvne.billing.UnlinkedAccount;
 ...
 */
@Deprecated
public class OldBillService {
	public static short DISC_REASON = 5;
	public static final BillingServiceInterfaceSoap port = KenanGatewayProvider.getInstance();
	private static final String USERNAME = "TSCPMVNE.BillService";
	private static Properties props = new Properties();
	private static final Logger logger = LoggerFactory.getLogger(OldBillService.class);

	public OldBillService() throws InitializationException {
		try {
			props.clear();
			props.load(OldBillService.class.getClassLoader().getResourceAsStream("com/tscp/mvne/config/legacy.properties"));
		} catch (IOException ioe) {
			logger.error("Exception initializing BillService.", ioe);
	...
	public void addComponent(
		...
		PkgComponent pkgComponent = new DefaultPkgComponent();
		if (componentid == null) {
		...
		if (messageHolder.getMessageHolder() != null && messageHolder.getMessageHolder().size() > 0) {
			for (MessageHolder message : messageHolder.getMessageHolder()) {
				if (message.getStatus().equals("Success")) {
					...
					throw new BillingException("addPackage", "Error adding component " + pkgComponent.getComponentId() + " to Account " + account.getAccountNo() + ". Returned message is " + message.getMessage());
				}
			}
		}
	}
		...
		if (valueHolder.getValueHolder() != null && valueHolder.getValueHolder().size() > 0) {
			for (ValueHolder value : valueHolder.getValueHolder()) {
				if (value.getStatusMessage().getStatus().equals("Success")) {
					logger.debug("Added Package " + iPackage.getId() + " :: Kenan returned ID -> " + value.getValue().trim() + " with IDServ -> " + value.getValue2());
					...
					throw new BillingException("addPackage", "Error adding package " + kenanPackage.getPackageId() + " to Account " + account.getAccountNo() + ". Returned message is " + value.getStatusMessage().getMessage());
				}
			}
		}
	}
	...
	public void addPayment(
			Account account, String paymentAmount) throws BillingException {
		...
		if (message != null) {
			logger.debug("Added Payment with Status [{}] and Message [{}]", message.getStatus(), message.getMessage());
			if (!message.getStatus().equals("Success")) {
				throw new BillingException("addPayment", "Error adding Payment $" + Double.parseDouble(paymentAmount) / 100 + " to Account " + account.getAccountNo() + ". Return Message is :: " + message.getMessage());
			}
		} else {
	...
	public void addServiceInstance(
		...
		if (!contains) {
			...
			if (message == null) {
				...
				if (!message.getStatus().equals("Success")) {
					throw new BillingException("addServiceInstance", "Error adding ServiceInstance " + serviceinstance.getExternalId() + " to account " + account.getAccountNo() + "..." + message.getMessage());
				}
			}
		} else {
			logger.error("Service is already associated with account {}, no service will be added.", account.getAccountNo());
			throw new ProvisionException("Service is already associated with this account...Skipping add...");
		}
	}

	private void bindAccountObject(
			Account account) {
		...
		if (response == null) {
			...
			if (response.getValue() == null || response.getValue().trim().length() == 0) {
				if (response.getStatusMessage() != null) {
					throw new BillingException("createAccount", "Account Number has not been returned.." + response.getStatusMessage().getStatus() + " " + response.getStatusMessage().getMessage());
				}
	...
	public void deleteServiceInstance(
		...
		DateTime discDate = new DateTime();
		discDate = discDate.plusDays(1);
		MessageHolder message = port.disconnectServicePackages(USERNAME, Integer.toString(account.getAccountNo()), serviceinstance.getExternalId(), serviceinstance.getExternalIdType(), DateUtils.getXMLCalendar(discDate), DISC_REASON);
		...
		if (message == null) {
			...
			logger.debug("Deleted Service with Status [{}] and Message [{}]", message.getStatus(), message.getMessage());
			if (!message.getStatus().equals("Success")) {
				throw new BillingException("deleteServiceInstance", "Error deleting ServiceInstance " + serviceinstance.getExternalId() + " to account " + account.getAccountNo() + "..." + message.getMessage());
			}
		}
	}
	...
	public Account getUnlinkedAccount(
		...
		Query q = session.getNamedQuery("fetch_unlinked_account");
		q.setParameter("in_cust_id", custId);
		List<UnlinkedAccount> unlinkedAccounts = q.list();
		session.getTransaction().rollback();

		for (UnlinkedAccount ua : unlinkedAccounts)
			try {
				return getAccountByAccountNo(ua.getAccountNo());
	...
	public Account getAccountByAccountNo(
			int account_no) throws BillingException {
	...
	private CustAddress getCustAddress(
			int accountno) {
	...
	public List<Package> getPackageList(
		...
		try {
			...
			logger.error("WS Exception thrown when calling getListActivePackages(\"username\"," + account.getAccountNo() + ")...." + ws_ex.getMessage());
	...
	public void reactivateBillingAccount(
	...
	public void updateAccount(
		...
		if (account.getContact_email() != null) {
			...
			if (message == null) {
				...
				if (!message.getStatus().equals("Success")) {
					throw new BillingException("updateAccount", "Error updating email address for account " + account.getAccountNo() + ". Returned message is " + message.getMessage());
				}
			}
		}
	}
	...
	public void updateAccountEmailAddress(
		...
		if (messageHolder != null)
			logger.debug("Updated account email with Status [{}] and Message [{}]", messageHolder.getStatus(), messageHolder.getMessage());
		else
	...
	public void updateServiceInstanceStatus(
		...
		// if (newThreshold != 0 && newThreshold != 5 && newThreshold != 6 && newThreshold != 7)
		...
		if (messageHolder == null) {
			...
			if (!messageHolder.getStatus().equals("Success"))
				...
				logger.info("Service " + serviceInstance.getExternalId() + " has been updated with new threshold value of " + newThreshold);
		}
	}
