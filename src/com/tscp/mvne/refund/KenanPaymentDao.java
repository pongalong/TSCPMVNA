...
public class KenanPaymentDao {
	...
	public static List<KenanPayment> getKenanPayments(
			Account account) throws RefundException {
	...
	public static void reversePayment(
			Account account, String amount, Date transDate, String trackingId) throws RefundException {
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		Transaction transaction = session.beginTransaction();
		...
		if (list == null) {
			...
			if (list.size() > 0) {
				if (!list.get(0).getStatus().equals("Y")) {
					transaction.rollback();
					throw new ContractException("reversePayment", "Error reversing payment " + trackingId + " on account " + account.getAccountNo() + ". Fail Reason is : " + list.get(0).getMsg());
				} else {
					transaction.commit();
				}
			} else {
	...
	public static void applyChargeCredit(
		...
		if (list == null) {
			transaction.rollback();
			session.close();
			throw new ContractException("applyChargeCredit", "Error applying pccharge credit on card " + creditCard.getNameOnCreditCard() + " " + creditCard.getCreditCardNumber());
		} else {
			if (list.size() > 0) {
				if (!list.get(0).getStatus().equals("Y")) {
					transaction.rollback();
					throw new ContractException("applyChargeCredit", "Error applying pccharge credit on card " + creditCard.getNameOnCreditCard() + " " + creditCard.getCreditCardNumber() + ". Fail Reason is : " + list.get(0).getMsg());
				} else {
					transaction.commit();
				}
			} else {
	...
	public static void applyChargeCredit(
			int accountNo, int trackingId, String amount, String refundBy, int refundCode, String notes) throws RefundException {
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		Transaction transaction = session.beginTransaction();
		try {
