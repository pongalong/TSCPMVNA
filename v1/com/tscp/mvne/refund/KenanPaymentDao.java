...
public class KenanPaymentDao {
	...
	public static List<KenanPayment> getKenanPayments(
		...
		try {
			...
			throw new OldRefundException("Error fetching Kenan Payments (getKenanPayments)");
		}
	}

	public static void reversePayment(
		...
		if (list == null) {
			transaction.rollback();
			session.close();
			throw new ContractException("Error reversing payment " + trackingId + " on account " + account.getAccountNo());
		} else {
			if (list.size() > 0) {
				if (!list.get(0).getStatus().equals("Y")) {
					transaction.rollback();
					throw new ContractException("Error reversing payment " + trackingId + " on account " + account.getAccountNo() + ". Fail Reason is : " + list.get(0).getMsg());
				} else {
					transaction.commit();
				}
			} else {
				transaction.rollback();
				throw new ContractException("Error reversing payment. No cursor items returned...");
			}
		}
	}

	public static void applyChargeCredit(
		...
		if (list == null) {
			transaction.rollback();
			session.close();
			throw new ContractException("Error applying pccharge credit on card " + creditCard.getNameOnCreditCard() + " " + creditCard.getCreditCardNumber());
		} else {
			if (list.size() > 0) {
				if (!list.get(0).getStatus().equals("Y")) {
					transaction.rollback();
					throw new ContractException("Error applying pccharge credit on card " + creditCard.getNameOnCreditCard() + " " + creditCard.getCreditCardNumber() + ". Fail Reason is : " + list.get(0).getMsg());
				} else {
					transaction.commit();
				}
			} else {
				transaction.rollback();
				throw new ContractException("Error applying pccharge credit. No cursor items returned...");
			}
		}
	}
		...
		try {
			...
			if (list == null) {
				...
				if (!list.get(0).getStatus().equals("Y")) {
					transaction.rollback();
					session.close();
					throw new ContractException("Error applying pccharge credit on card " + ". Fail Reason is : " + list.get(0).getMsg());
				} else {
					transaction.commit();
				}
			} else {
			...
			transaction.rollback();
			throw new OldRefundException("applyChargeCredit", he.getMessage());
		} finally {
			if (session.isOpen()) {
				try {
					session.close();
